...
import Log , { init as initLog }from '../../../../lib/connect/src/js/utils/debug';
import { CoreMessage } from '../../types/params';
...
let _core: CoreMessage;

let _messageID: number = 0;
export const messagePromises: {
    [K in number]: $T.Deferred<any>;
} = {};
...
export const call = async (params: Object): Promise<Object> => {
    ...
    try {
        const response: Object | null = await postMessage({ type: IFRAME.CALL, payload: params });
        if (response) {
            return response;
        } else {
...
export const renderWebUSBButton = (className: string | null): void => {
...
export const customMessage: $T.API["customMessage"] = async (params) => {
...
export const requestLogin: $T.API["requestLogin"] = async (params) => {
